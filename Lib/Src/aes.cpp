...
void Aes::initialize_multiplytables() { //! Not used (slow) wrong implementation
  array<uint8_t, 256> In_Calcul;
  array<int, 7> multiplications = {1, 2, 3, 9, 11, 13, 14};
  int table;
  for (size_t t = 0; t < multiplications.size(); t++) {
...
Aes::key_t Aes::gen_key() {
  ...
  random_device os_seed; // Use /dev/urandom
  // TODO Read More Info About this
  // Safe Rng on linuxOS based and WindowsOS but not others
...
void Aes::ShiftRow(block_t &word, int line, int nbshift) {
  ...
  while (nbshift % 4) {
    ...
    for (int i = start_index; i < end_indice; i++) {
      if (i + 1 == end_indice)
        word[i] = start;
      else
        word[i] = word[i + 1];
...
void Aes::Padding(block_t &block, random_device &rand) {
  switch (Mode_Pad) {
  case BASIC:
    block.fill(0);
    break;
...
template <typename T>
T Aes::Decrypt(vector<Aes::block_t> to_decrypt, const key_t &key) {
  switch (Mode_Enc) {
  case ECB:
...
template vector<Aes::block_t>
Aes::Encrypt<unsigned char *>(unsigned char *to_encrypt, const key_t &key,
                              size_t size);
template unsigned char *Aes::Decrypt<unsigned char *>(vector<Aes::block_t>,
                                                      const key_t &key);

template unsigned char *Aes::fromblock(vector<block_t> blocks);
...
/*  int main(int argc, const char **argv) {
  ...
  Crypt.Mode_Enc = Aes::CBC;
  string boom = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
  ...
  int max = 1000000;
  int *test = (int *)malloc(max * sizeof(int));
  for (size_t i = 0; i < max; i++) {
    test[i] = i + 1;
  }
  auto cypher_array = Crypt.Encrypt(test, key, sizeof(int) * max);
  free(test);
  int *dec_array = Crypt.Decrypt<int *>(cypher_array, key);
  cout << dec_array[max - 1] << endl;
  free(dec_array);
  return 0;
  // Compile with g++ -std=c++17 -finline-functions -funroll-loops -mtune=native
  // -flto -O3 -g aes.c++ -o aes && aes
}   */